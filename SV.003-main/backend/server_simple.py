...
async def security_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/") and not auth_security.is_public_api_route(
        ...
        if not vet_id and not auth_security.allow_insecure_vet_header():
            flow_vet_id = auth_security.try_authenticate_cedula_flow_request(request)
            if flow_vet_id:
                vet_id = flow_vet_id
            else:
                return JSONResponse(
    ...
    if env in ("production", "prod") or request.url.scheme == "https":
...
def _cedula_auth_gate_response(
    veterinarian: dict,
    *,
    send_upload_reminder: bool = False,
) -> Optional[dict]:
    """Devuelve payload de cédula si el perfil no debe recibir JWT completo."""
    ced_status = (
        (veterinarian.get("cedula_verification_status") or "").strip()
        or CEDULA_STATUS_UNSUBMITTED
    )
    ...
    if ced_status == CEDULA_STATUS_REJECTED:
        return _cedula_flow_response(
            veterinarian,
            message="Tu registro profesional fue rechazado. Revisa tus datos y vuelve a intentar.",
            needs_upload=True,
            verification_status=ced_status,
            can_skip=False,
        )
...
def _require_consultation_owned(consultation_id: str, vet_id: str) -> dict:
    ...
    if (cons.get("user_id") or "").strip() != vet_id.strip():
        raise HTTPException(
            status_code=403,
            detail="No autorizado para acceder a esta consulta",
        )
    return cons
...
async def login_veterinarian(credentials: VeterinarianLogin, request: Request):
    ...
    # Gating: manejar estados de verificación de cédula antes de cualquier JWT completo.
    cedula_gate = _cedula_auth_gate_response(veterinarian, send_upload_reminder=True)
    if cedula_gate:
        return cedula_gate
...
async def verify_2fa(verification: TwoFactorVerify, request: Request):
    ...
    cedula_gate = _cedula_auth_gate_response(veterinarian)
    if cedula_gate:
        return cedula_gate
...
async def verify_cedula(
    ...
    vet_id = _require_vet_id(x_veterinarian_id)
    if payload.veterinarian_id and payload.veterinarian_id.strip() != vet_id:
...
async def get_consultation_history(
    ...
    vet_id = _require_vet_id(x_veterinarian_id)
    if veterinarian_id.strip() != vet_id:
        raise HTTPException(status_code=403, detail="No autorizado para este historial")
...
async def update_consultation_payload(
    ...
    consultation = _require_consultation_owned(consultation_id, vet_id)
...
async def set_consultation_rating(
    ...
    vet_id = _require_vet_id(x_veterinarian_id)
    _require_consultation_owned(consultation_id, vet_id)
...
async def get_checkout_status(session_id: str, x_veterinarian_id: str = Header(None)):
    """Obtiene estado del pago del usuario autenticado."""
    ...
    _require_payment_session_owned(transaction, vet_id)
...
async def interpret_medical_image(
    ...
    vet_id = _require_vet_id(x_veterinarian_id)
    if request.veterinarian_id and request.veterinarian_id.strip() != vet_id:
        raise HTTPException(status_code=403, detail="La identidad no coincide con la sesión")
