...
## 0.E Project override — GUIAA (guiaa.vet)

When working in this repository, read `DESIGN.md` and `PRODUCT.md` first. Use skill `guiaa-taste` for routing.

- **Landing only** (`LandingPage`, marketing, `/`): trust-first B2B clínica, not Awwwards chaos.
- **Never apply this skill** to `/app/*` clinic dashboards, consultation flows, or data tables — use Impeccable + `DESIGN.md` there.
- **Brand colors are fixed:** navy `#0c2d4d`, blue `#265B93`, green `#3d9b8f`. Do not swap to purple gradients or cream/beige AI defaults.
- **Typography:** system-ui stack per `DESIGN.md`; do not force Geist/Inter swaps on clinic screens.
...
**Baseline (GUIAA landing):** `5 / 5 / 3`. **Global Taste default:** `8 / 6 / 4`. Use GUIAA baseline in this repo unless the design read overrides. Do not ask the user to edit this file - overrides happen conversationally.
